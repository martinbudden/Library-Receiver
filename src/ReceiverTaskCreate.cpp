...
ReceiverTask* ReceiverTask::createTask(task_info_t& taskInfo, ReceiverBase& receiver, ReceiverWatcher* receiverWatcher, uint8_t priority, uint8_t coreID) // NOLINT(readability-convert-member-functions-to-static)
{
#if defined(RECEIVER_TASK_IS_NOT_INTERRUPT_DRIVEN)
    assert(false && "Task interval not specified for ReceiverTask");
#endif
...
ReceiverTask* ReceiverTask::createTask(task_info_t& taskInfo, ReceiverBase& receiver, ReceiverWatcher* receiverWatcher, uint8_t priority, uint8_t coreID, uint32_t taskIntervalMicroSeconds) // NOLINT(readability-convert-member-functions-to-static)
{
    static ReceiverTask receiverTask(taskIntervalMicroSeconds, receiver, receiverWatcher);
